...
### (Опционально) VITE_ADMIN_UID / VITE_ADMIN_UIDS
Нужно, чтобы в админку могли входить только пользователи из allowlist (иначе вход будет запрещён сообщением про права администратора).

- **Вариант 1 (один админ)**:
  - **Name**: `VITE_ADMIN_UID`
  - **Secret**: UID администратора из Firebase Authentication
- **Вариант 2 (несколько админов)**:
  - **Name**: `VITE_ADMIN_UIDS`
  - **Secret**: UID-ы через запятую, например: `UID1,UID2`
...
- Важно понимать: любые переменные `VITE_*` попадают в собранный фронтенд и **видимы в браузере**.
  - Firebase Web config (apiKey/authDomain/...) — это **не секрет** по дизайну Firebase.
  - Реальная безопасность обеспечивается **Firestore/Storage Rules** и allowlist админов.
- Не храните в `VITE_*` настоящие приватные ключи/токены.
